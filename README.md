...
- This project dives into how land is utilized in Wisconsin. The original dataset includes data on the entire United States, however, for this project, I focused solely on the state of Wisconsin. 
- Created a python module that pulled the data from a SQLite3 database, a zip file, and numpy matrices representing land use.  
- Created a connection class with methods that allowed you to return the names, years, and various land usage code arrays from specific numpy files. 
...
- Created a plotting method that allows one to pass in a city and see every land usage code and its respective % makeup of the city over the years. 
...
- Sampled a percentage of zip file containing data for 1/01/2019 as a csv file and added a country column that linked each IP address to its corresponding country using regex.
...
- Produced a Browse page bringing one to a dataframe hostin data such as the song name, artist(s), producer(s) and writer(s). 
- Created an API page that allows one to query their own data to get custom information from the data. 
- Included a donate link and an email subscription option that allows one to sign up for a mailing list. 
...
- Designed two search methods, dfs_pass and bfs_pass, thta utilize depth-first search and binary-first search on the website's homepage to collect hidden letters and concatenate them to seperate passwords.
...
- The repository I analyzed contained a python program wc.py that counted the words of a given text passed in. 
- Analzyed how the .py file had grown and changed over time, including who had made what changes to each iteration of repository. 
- Passed three test strings into my own function "run_wc" that ran every commit of wc.py to see which commits matched the expected output and which did not. 
- Randomly generated words X words and chose Y unique words to run the different versions of wc.py on, in order to test the performance of the various commits.
...
- Put the data into Pandas dataframes and analyzed various things, including the most landlocked countries, the countries with the highest/lowest birth/death rates, and capital's population density and distance to their closest coast. 
...
- Generated interesting scatterplots, such as phone usage on GDP per capita or GDP per capita on literacy rates. 
